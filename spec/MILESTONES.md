...
- **구조적 파싱 성공률** = ok/(ok+malformed) < config.pilot.parse_success_threshold(0.90)인
  모델은 **자동 제외**하고 기록
  - (재정의 2026-07-30: no_call 은 파서 문제가 아니라 모델 행동 — 원문 확인 결과 정보 부족
    쿼리 되물음이며 절반이 random_k(무관 후보) 조건 발생. func/strict 0점으로 채점에 이미
    반영되므로 게이트에서 분리, 비율은 행동 지표로 보고·결론 기술.)
