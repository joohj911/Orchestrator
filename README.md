...
- **example 누출 컷 threshold = 0.92** (0.90 에서 owner 결정으로 상향). example 은 test 쿼리를
  참조하지 않고 tool 메타만으로 작성돼 진짜 누출은 없으며, 남은 근접(최대 0.9189)은 e5 가 같은
  좁은 의도의 짧은 문장을 필연적으로 0.90~0.92 로 임베딩하는 성질(anisotropy) 때문이다. 억지로
  더 낮추면 부자연스러운 example 이 되어 dense_multi 표현 품질만 저하되므로, 자연스러움을 유지하고
  컷을 0.92 로 둔다(여전히 '거의 동일'만 차단).
